...
const PAN_PATTERN = /^[0-9]{6}[*X]{3,9}[0-9]{4}$/;
...
function OpenDisputeModal({ onClose, onCreated }) {
  ...
  const validate = () => {
    ...
    else if (!PAN_PATTERN.test(form.panMasked.trim()))
      e.panMasked = 'Format: 411111****1111 (first 6, 3–9 masked, last 4)';
    if (!form.reasonCode)
        ...
        <div className="modal-content">
          ...
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              ...
              {/* Masked PAN */}
              <div className="mb-3">
                <label className="form-label small fw-semibold">Masked PAN <span className="text-danger">*</span></label>
                <input
                  type="text"
                  ...
                  onChange={e => set('panMasked', e.target.value)}
                  disabled={submitting}
                />
                {errors.panMasked
                  ? <div className="invalid-feedback">{errors.panMasked}</div>
                  : <div className="form-text">First 6 digits, 3–9 masked (* or X), last 4 digits</div>
                }
              </div>
